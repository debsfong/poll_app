...
def movie_names_before_1940
  ...
  Movie.select('title', 'id', 'yr')
  .where('movies.yr <1940')
end

def eighties_b_movies
	# List all the movies from 1980-1989 with scores falling
  # between 3 and 5 (inclusive). Show the id, title, year, and score.
...
def vanity_projects
  ...
  #also appeared as the starring actor. Show the movie id and title and director's name.
...
def starring(whazzername)
	...
	# A name is like whazzername if the actor's name contains all of
  #the letters in whazzername, ignoring case, in order.

	# ex. "Sylvester Stallone" is like "sylvester" and "lester stone"
  # but not like "stallone sylvester" or "zylvester ztallone"
  name = whazzername.split(" ").join("").split("").join('%')
  name = "%" + name + "%"
  Movie.select('*')
    .joins(:actors)
